...
extension Object {
    ...
    public func addNotificationBlock(_ block: @escaping (ObjectChange) -> Void) -> NotificationToken {
        return RLMObjectAddNotificationBlock(self, { names, oldValues, newValues, error in
            ...
            guard let names = names, let oldValues = oldValues, let newValues = newValues else {
                block(.deleted)
                return
            }

            block(.change(zip(names, zip(oldValues, newValues)).map { name, value in
                PropertyChange(name: name, oldValue: value.0, newValue: value.1)
            }))
        })
    }
}
